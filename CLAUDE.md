...
| Armellini Post | `/api/armellini-post` (`expoflor_operaciones_cajas`, `armellini_consignees`, `armellini_exports`) | genera el XML AelisShipperEDI de Armellini desde el XML de operaciones de Expoflor. Reemplaza el proyecto externo "ArmelliniFormat". `<Invoice>` sale de `dartis_ventas.id_comercializadora` (no del campo `factura` del XML, que lleva el pedido); las cajas se seleccionan por destino con consignee cargado, no por carrier (las de Heinen's vienen como `HEB` = "ARMELLINI NO EDI"). Incluye la pre-alerta de despacho por correo, con destinatarios y días de entrega por destino. Usa la **API de Gmail sobre HTTPS** (no SMTP, que Render bloquea en el plan gratuito): requiere `GMAIL_CLIENT_ID`/`GMAIL_CLIENT_SECRET`/`GMAIL_REFRESH_TOKEN`/`GMAIL_USER`, que se obtienen con `backend/scripts/gmail_autorizar.py` |
